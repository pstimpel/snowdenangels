...
<div id="accordion">
    ...
    <div>
        <p>
            We are using a so called CPU mining software, called <a href="https://github.com/fireice-uk/xmr-stak" target="_blank">XMR-Stak</a>. This little program is calculating so called Hashes, and helps to maintain the network behind <a href="https://getmonero.org/" target="_blank">Monero</a>. We wrote some support stuff around it, created a graphical user interface, and some statistics, so the user who would like to join this little project doesn't face all the hassle of setting up a miner. And, we take care not to overload your PC, by using only a very small share of the CPUs power. The software we wrote is licensed under the GPL, and is open source. We host it at <a href="https://github.com/pstimpel/snowdenangels" target="_blank">https://github.com/pstimpel/snowdenangels</a>. If you would like to check the sources, don't hesitate. If you want to help, join us on Github. Every help is much appreciated!
        </p>
    </div>
    <h3>What are the system requirements to run this software?</h3>
    <div>
        <p>
The whole project is available to run on Microsoft Windows (Vista, 7, 8, 10), but needs a 64bit environment. We are working on providing it for 32bit environments as well. You need to have an Internet connection, that's it! Oh, Microsoft .net Framwork has to be installed, but this is given in most cases. The setup program will tell you if the framework is missing.
        </p>
    </div>
...
Yes, Windows might warn you about installing software from unknown sources. All we can do for the moment is to tell you: the software was checked by several Antivirus programs before we uploaded it, and the sources are available for further security checks as well. So finally, it is on you to trust us or not.</p>
    ...
    <div>
        <p>
            No problem, just hit me on <a href="https://twitter.com/PjotrS72" target="_blank">https://twitter.com/PjotrS72</a> or shot me an email: <a href="mailto:pstimpel@googlemail.com">pstimpel@googlemail.com</a>, PGP available on key-servers.
